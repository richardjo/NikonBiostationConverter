...
converts the image files from a png format into the open source ome.tiff format and writes metadata from the csv data files to the image files. Files can also be renamed in order to support Fiji's ([https://fiji.sc/](https://fiji.sc/)) stitching plug-in.
